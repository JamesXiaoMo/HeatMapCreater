...
class HeatMapCreator:
    ...
    def callback(
        ...
        if self.counter == 0 and len(self.raw_grid_map_data_2d) == 0:
            ...
            self.raw_grid_map_full_update(
                raw_grid_map_data,
                raw_grid_map_width_pixel,
                raw_grid_map_height_pixel,
                raw_grid_map_resolution,
                raw_grid_map_origin_x,
                raw_grid_map_origin_y,
            )
        else:
            # 非第一次传入数据
    ...
    def raw_grid_map_part_update(
        ...
        """
        # 计算地图扩展的距离
        delta_x_negative = self.raw_grid_map_origin_x - raw_grid_map_origin_x   # x轴负方向扩展距离
        delta_y_negative = self.raw_grid_map_origin_y - raw_grid_map_origin_y   # y轴负方向扩展距离
        delta_x_positive = raw_grid_map_width_pixel - self.raw_grid_map_width_pixel - delta_x_negative  # x轴正方向扩展距离
        delta_y_positive = raw_grid_map_height_pixel - self.raw_grid_map_height_pixel - delta_y_negative    # y轴正方向扩展距离
